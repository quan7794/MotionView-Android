...
public class TextEntity extends MotionEntity {
    ...
    private void updateRealEntity(boolean moveToPreviousCenter) {
...
//        }
        ...
        @SuppressWarnings("UnnecessaryLocalVariable")
        float widthAspect = 1.0F * canvasWidth * Helper.INSTANCE.getRatio() / width;

        // for text we always match text width with parent width
        this.realHolyScale = widthAspect;
    ...
    @Override
    protected void drawRealContent(@NonNull Canvas canvas, @Nullable Paint drawingPaint) {
        if (bitmap != null) {
