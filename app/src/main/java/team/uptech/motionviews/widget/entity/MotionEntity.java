...
public abstract class MotionEntity {
    ...
    protected final Matrix realMatrix = new Matrix();
    /**
    ...
    protected void updateRealMatrix() {
        // init matrix to E - identity matrix
        realMatrix.reset();

        float topLeftX = layer.getX() * canvasWidth * Helper.INSTANCE.getRatio();
        float topLeftY = layer.getY() * canvasHeight * Helper.INSTANCE.getRatio();

        float centerX = topLeftX + getWidth() * realHolyScale * 0.5F;
        float centerY = topLeftY + getHeight() * realHolyScale * 0.5F;
    ...
    public float absoluteRealCenterX() {
        float topLeftX = layer.getX() * canvasWidth * Helper.INSTANCE.getRatio();
        return topLeftX + getWidth() * realHolyScale * 0.5F;
    }

    public float absoluteRealCenterY() {
        float topLeftY = layer.getY() * canvasHeight * Helper.INSTANCE.getRatio();
        return topLeftY + getHeight() * realHolyScale * 0.5F;
    }

    public PointF absoluteRealCenter() {
        float topLeftX = layer.getX() * canvasWidth * Helper.INSTANCE.getRatio();
        float topLeftY = layer.getY() * canvasHeight * Helper.INSTANCE.getRatio();
    ...
    public final void drawReal(@NonNull Canvas canvas, @Nullable Paint drawingPaint) {
    ...
    protected abstract void drawRealContent(@NonNull Canvas canvas, @Nullable Paint drawingPaint);
